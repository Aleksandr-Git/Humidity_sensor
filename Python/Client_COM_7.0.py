...
# !!! В СКЕТЧЕ АРДУИНО ОТКЮЧЕНА ПЕДЕДАЧА ПОКАЗАНИЙ ДАТЧИКОВ ВЛАЖНОСТИ
...
#                    push_msg = DATA
                    push_msg = 'Показания'  # ИЗМЕНИТЬ НА ДАННЫЕ ОТ КОНТРОЛЛЕНА DATA
                    Thread(target=push, args=(push_msg, '\n')).start()
                    break
...
def inet():
    ...
    try:
        ...
        if Flag == 0:  # срабатывает при восстановлении связи
            ...
            Flag = 1
#            push_msg = '\nСвязь отсутствовал с ' + T_1 + ' до ' + t_1 + ' ' + DATA.decode().rstrip()
            push_msg = '\nСвязь отсутствовал с ' + T_1 + ' до ' + t_1 + ' '  # ДОБАВИТЬ ДАННЫЕ С ДАТЧИКОВ
